...
try:
    ...
    raise SystemExit(
        "This dashboard requires Streamlit. Install it with "
        "'python -m pip install streamlit' and run the app again."
    ) from exc
try:
    pd = importlib.import_module("pandas")  # Used to work with tabular data
except ModuleNotFoundError as exc:
    raise SystemExit(
        "This dashboard requires pandas. Install it with "
        "'python -m pip install pandas' and run the app again."
    ) from exc
...
random.seed(42)  # Setting a seed so results are consistent every time you run
sample_data = {
    'Product': ['Product A', 'Product B', 'Product C', 'Product D'],
    'Sales': [random.randint(100, 499) for _ in range(4)],   # Random sales numbers
    'Profit': [random.randint(20, 99) for _ in range(4)]     # Random profit numbers
}
...
st.bar_chart(df.set_index('Product')[['Sales', 'Profit']])  # Render a grouped bar chart